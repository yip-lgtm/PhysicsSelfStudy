...
```mermaid
graph LR
    A[Literature Review] --> B[Identify Gap]
    B --> C[Form Hypothesis]
    C --> D[Design Method]
    D --> E[Collect Data]
    E --> F[Analyze]
...
```mermaid
mindmap
  root((Directed Studies & Seminars))
    X-courses
      PHYS 2090X Year 1-2
      PHYS 3090X Year 2-3
      PHYS 4090X Year 3-4
...
```mermaid
graph TD
    A[Literature review] --> B[Gap identification]
    B --> C[Supervisor meeting]
    C --> D[Project proposal]
    D --> E[Data collection]
    E --> F[Analysis]
    F --> G{Results?}
    G -->|Positive| H[Paper draft]
    G -->|Null/ambiguous| I[Revise method]
    I --> E
    H --> J[Submit]
    J --> K[arXiv + journal]
    K --> L[Present]
...
```mermaid
graph TD
    A[First meeting] --> B[Set expectations]
    B --> C[Weekly meetings]
    C --> D{Monthly: progress}
    D -->|On track| E[Continue]
    D -->|Behind| F[Address issues]
    E --> G[End of project]
    F --> C
    G --> H[Deliverable]
    H --> I[Letter of rec]
