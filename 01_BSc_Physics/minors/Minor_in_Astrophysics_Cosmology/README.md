...
```mermaid
graph TD
    A[Parallax 100pc] --> B[Cepheid 30Mpc]
    B --> C[Type Ia 1Gpc]
    C --> D[Hubble z 0.1]
    D --> E[BAO z 0.5]
    E --> F[CMB z 1100]
...
```mermaid
graph TD
    A[Big Bang 0] --> B[Inflation]
    B --> C[BBN 1s]
    C --> D[CMB 380kyr]
    D --> E[First stars 100Myr]
    E --> F[Galaxies 1Gyr]
    F --> G[Now 13.8Gyr]
```
...
```mermaid
graph TD
    A[Inspiral] -->|h+| B[Merger]
    B -->|ringdown| C[Final BH]
    D[LIGO] -->|interferometer| E[Strain]
    E -->|template match| F[Detection]
```

## 📊 Diagram 5: Multi-Messenger
```mermaid
graph TD
    A[Multi-messenger] --> B[Photons]
    A --> C[Neutrinos]
    A --> D[Cosmic rays]
    A --> E[Gravitational waves]
    B --> F[Electromagnetic]
    C --> F
    D --> F
    E --> F
