...
```mermaid
graph TD
    A[Energy] --> B[Fossil: coal/oil/gas]
    A --> C[Nuclear: fission/fusion]
    A --> D[Renewable: solar/wind/hydro]
    B --> E[CO2 emissions]
    C --> F[Low CO2, waste issue]
    D --> G[Intermittency, storage]
    E --> H[Climate change]
    F --> I[Long-term solution]
    G --> J[Grid + storage]
```
...
```mermaid
graph TD
    A[EM waves] --> B[Radio: AM/FM/WiFi]
    A --> C[Microwave: oven/radar/5G]
    A --> D[IR: thermal imaging]
    A --> E[Visible: displays]
    A --> F[UV: sterilization]
    A --> G[X-ray: medical/security]
    A --> H[Gamma: PET/radiotherapy]
```
...
graph TD
    ...
    A --> F[Composite]
    B --> G[Conduct, ductile]
    C --> H[Brittle, insulating]
    D --> I[Flexible, lightweight]
    E --> J[Doped for conductivity]
    F --> K[Anisotropic, tailored]
```

## 📊 Diagram 5: Climate System
```mermaid
graph TD
    A[Sun] -->|shortwave| B[Earth]
    B -->|absorbed| C[Heating]
    B -->|emitted LW| D[GHG absorb]
    D -->|re-emit| E[Warming]
    C --> F[Temperature]
    E --> F
    F -->|feedback| G[Ice-albedo, water vapor]
    G --> E
```
