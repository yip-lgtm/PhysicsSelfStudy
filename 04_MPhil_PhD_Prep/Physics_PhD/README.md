...
gantt
    ...
    section Qualifying
    Exam prep :b1, after a1, 6M
    Exam :b2, after b1, 1M
    section Research
    Lit review :c1, after b2, 6M
    Method :c2, after c1, 6M
    Data :c3, after c2, 12M
    Analysis :c4, after c3, 6M
    section Writing
    Draft :d1, after c4, 6M
    Revise :d2, after d1, 3M
    section Defense
    Submit :e1, after d2, 1M
    Defense :e2, after e1, 2M
```

## 📊 Diagram 3: Research Cycle
```mermaid
graph TD
    A[Question] --> B[Lit review]
    B --> C[Hypothesis]
    C --> D[Method]
    D --> E[Data]
    E --> F[Analysis]
...
graph TD
    ...
    B -->|Academia| C[Postdoc 2-3 yr]
    C --> D[Assistant Prof]
    B -->|Industry| E[R&D Scientist]
    E --> F[Senior, Principal]
    B -->|Finance| G[Quant]
    B -->|Consulting| H[Tech consulting]
    B -->|Government| I[National lab, policy]
```

## 📊 Diagram 5: Advisor-Student Fit
```mermaid
graph TD
    A[Advisor fit] --> B[Research match]
    A --> C[Style: hands-on vs hands-off]
    A --> D[Communication: weekly vs monthly]
    A --> E[Funding: stable vs grant-dependent]
    A --> F[Alumni: placement record]
    B --> G[Good fit]
    C --> G
    D --> G
    E --> G
    F --> G
    G --> H[5-year relationship success]
```
